 ...
 * @return array
 */
add_filter( 'umami_connect_get_all_events', function( $events, $per_page = 25 ) {
	...
	foreach ( $forms as $form_id ) {
		...
		if ( is_string( $pairs_raw ) && $pairs_raw !== '' ) {
			...
			if ( is_array( $decoded ) ) {
				foreach ( $decoded as $k => $v ) {
					...
					if ( '' === $k ) { continue; }
					$pairs[] = array( 'key' => $k, 'value' => (string) $v );
				}
			}
		}
		...
		$events[] = array(
			...
			'edit_link'          => admin_url( 'admin.php?page=wpforms-builder&view=settings&form_id=' . (int) $form_id . '&section=umami_tracking' ),
