...
add_filter('the_content', function($content) {
    ...
    if ($autotrack_links) {
        ...
        foreach ($links as $link) {
            ...
            $relVal = $link->hasAttribute('rel') ? $link->getAttribute('rel') : '';
            if ($relVal && preg_match('/(^|\s)umami:[a-z0-9\-]+/i', $relVal)) {
                continue;
            }
            ...
            foreach ($link->attributes as $attr) {
                if (strpos($attr->name, 'data-umami-event-') === 0) { $hasAnyPair = true; break; }
            }
            if ($hasAnyPair) {
                continue;
            }
