	...
	switch ( $tab ) {
		case 'setup':
			$screen->add_help_tab(
				array(
					'id'      => 'umami_help_setup',
					'title'   => wp_kses_post( 'Setup', 'umami-connect' ),
			...
			$screen->add_help_tab(
				array(
					...
					'title'   => wp_kses_post( 'Settings', 'umami-connect' ),
					'content' => wp_kses_post(
		...
		case 'share-url':
			$screen->add_help_tab(
				array(
					...
					'title'   => wp_kses_post( 'Overview', 'umami-connect' ),
					'content' => wp_kses_post(
...
function umami_connect_add_help_self_protection() {
	...
	$screen->set_help_sidebar(
		'<p><strong>' . wp_kses_post( 'Support & Resources', 'umami-connect' ) . '</strong></p>' .
		'<p><a href="https://github.com/' . UMAMI_CONNECT_GITHUB_USER . '/' . UMAMI_CONNECT_GITHUB_REPO . '" target="_blank">GitHub</a></p>' .
		'<p><a href="' . UMAMI_CONNECT_DISCORD_INVITE . '" target="_blank">Discord</a></p>' .
		'<p><a href="https://umami.is/docs" target="_blank">' . wp_kses_post( 'Umami Documentation', 'umami-connect' ) . '</a></p>'
	);
}
...
function umami_connect_add_help_events_overview() {
	...
	$screen->add_help_tab(
		array(
			...
			'title'   => wp_kses_post( 'Creating Events', 'umami-connect' ),
			'content' => wp_kses_post(
	...
	$screen->add_help_tab(
		array(
			...
			'title'   => wp_kses_post( 'Managing Events', 'umami-connect' ),
			'content' => wp_kses_post(
	...
	$screen->add_help_tab(
		array(
			...
			'title'   => wp_kses_post( 'Best Practices', 'umami-connect' ),
			'content' => wp_kses_post(
...
function umami_connect_add_help_advanced() {
	...
	$screen->set_help_sidebar(
		...
		'<p><a href="https://github.com/' . UMAMI_CONNECT_GITHUB_USER . '/' . UMAMI_CONNECT_GITHUB_REPO . '" target="_blank">GitHub</a></p>' .
		'<p><a href="' . UMAMI_CONNECT_DISCORD_INVITE . '" target="_blank">Discord</a></p>' .
		'<p><a href="https://umami.is/docs/tracker-configuration" target="_blank">' . wp_kses_post( 'Tracker configuration', 'umami-connect' ) . '</a></p>'
	);
		...
		case 'host-url':
			...
			$screen->add_help_tab(
				array(
					...
					'title'   => wp_kses_post( 'Usage', 'umami-connect' ),
					'content' => sprintf(
...
function umami_connect_add_screen_options_events_overview() {
	...
	add_screen_option(
		'per_page',
		array(
			'label'   => wp_kses_post( 'Events per page', 'umami-connect' ),
			'default' => 20,
...
function umami_connect_events_overview_columns( $columns ) {
	return array(
		'event'       => wp_kses_post( 'Event', 'umami-connect' ),
		'integration' => wp_kses_post( 'Integration', 'umami-connect' ),
		'post'        => wp_kses_post( 'Source', 'umami-connect' ),
		'block_type'  => wp_kses_post( 'Block Type', 'umami-connect' ),
		'label'       => wp_kses_post( 'Label/Text', 'umami-connect' ),
		'data_pairs'  => wp_kses_post( 'Data Pairs', 'umami-connect' ),
	);
}
